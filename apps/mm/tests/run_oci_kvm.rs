...
fn mm_run_boots_an_oci_image() {
    ...
    // A helper to drive `mm` subcommands against this run's isolated MM_ROOT.
    ...
    // store round-trip unit test. We drive these *after* the foreground `mm run` is
    // gone: the registry is `redb`, which holds an exclusive lock while a process has
    // it open, so a concurrent `mm ps` during a foreground run can't read it (in
    // normal use you'd `mm run -d`, which returns and releases the lock). The record
    // persists — with its allocated IP — until `mm rm` deletes it, so `mm ps` /
    // `mm ps --ip-only` see it here, and the final `mm ps` must not.
    let (ps_ok, ps_listed) = mm(&["ps"]);
    ...
    // Lifecycle (FR-8/FR-9): `mm ps` lists the machine with its allocated IP, and
    // `mm rm` removes it so it no longer appears.
    assert!(ps_ok, "`mm ps` failed");
    assert!(
        ps_listed.contains("oci-test"),
        "`mm ps` did not list the machine:\n{ps_listed}"
    );
    assert!(
