...
- [🟡] Create a machine via REST → scheduler places it → agent reconciles and boots it on a Linux/KVM host (FR-17, FR-18, FR-19) — **REST create → `scheduler::place` → assignment pushed to the agent over mTLS → agent reports `Running` → status converges is verified end-to-end** by `reconcile_loop.rs` (real Postgres + real mTLS gRPC, ~1.7 s). The agent's *actual* boot reuses the KVM-verified `mm_host::launch` path (the same one `mm run` uses — green in the M1 KVM jobs), but the full controller→agent→**real-VM-on-KVM** chain is not yet run in CI: the cluster test uses an in-process fake agent that reports `Running` without booting. This is **closeable in a single CI job** — `mm-controller` + a real `mm-agent` + boot busybox on the one `ubuntu-latest` `/dev/kvm` runner (the reason `mm-host` was extracted) — not a multi-host requirement; it is simply not yet attempted.
- [🟡] Desired + observed state persist in Postgres; data plane survives a control-plane restart with no duplicate assignment (FR-22, NFR-R2) — **what is actually verified** by `reconcile_loop.rs`: control-plane state persists in Postgres across an *in-process simulated* restart (a fresh `AgentRegistry` + reconcile read), and a `Running` machine yields `decide → None` so no assignment is re-issued (idempotency). **Not yet exercised:** a real controller *process* restart, and "data plane survives" with an actual booted VM — both fold into the cluster-boot e2e above.
