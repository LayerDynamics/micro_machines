...
- [🟢] A real microVM **boots to userspace on KVM** (FR-1, FR-2, FR-3 blk/vsock/serial, FR-6 ext4 rootfs) — **verified: the `kvm-integration` job passes**. The guest boots the kernel (64-bit boot protocol, kvm-clock, in-kernel irqchip+PIT), mounts an ext4 rootfs over virtio-blk, runs `mm-init` as PID 1, and signals readiness over **virtio-vsock**; boot-to-userspace **p50 ≈ 1.02 s** (min 0.98 s, max 1.10 s over 30 iterations). virtio-net is exercised by the separate `net-integration` job (the boot fixture itself has no net device). `mm run`'s OCI→rootfs orchestration is **runtime-verified** by the `mm-run-integration` job, which pulls a real OCI image (`docker.io/library/busybox`) with skopeo+umoci, builds the RO-base + ephemeral-overlay rootfs, injects mm-init, and boots it to userspace.
